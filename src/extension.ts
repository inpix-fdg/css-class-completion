...
enum Command {
  Cache = "ix-css-class-completion.cache",
}

enum Configuration {
  IncludeGlobPattern = "ix-css-class-completion.includeGlobPattern",
  ExcludeGlobPattern = "ix-css-class-completion.excludeGlobPattern",
  EnableEmmetSupport = "ix-css-class-completion.enableEmmetSupport",
  HTMLLanguages = "ix-css-class-completion.HTMLLanguages",
  CSSLanguages = "ix-css-class-completion.CSSLanguages",
  JavaScriptLanguages = "ix-css-class-completion.JavaScriptLanguages",
}
    ...
    languageSelector,
    {
      provideCompletionItems(
        ...
        const completionItems = uniqueDefinitions.map((definition) => {
          ...
          // CSS 코드 블록 삽입
          const cssCode = `${definition.declarations}`;

          markdownString.appendCodeblock(cssCode, "css");

          const workspaceFolders = workspace.workspaceFolders;
          if (workspaceFolders) {
            for (const folder of workspaceFolders) {
              if (
                definition.location?.uri.fsPath.startsWith(folder.uri.fsPath)
              ) {
                const relativePath = definition.location?.uri.fsPath.replace(
                  folder.uri.fsPath,
                  ""
                );
                markdownString.appendMarkdown(
                  `[${relativePath}](${definition.location.uri})`
                );
                break;
              }
            }
          ...
          // completionItem.detail = definition.declarations;
...
const registerHTMLProviders = (disposables: Disposable[]) =>
  workspace
    ...
    ?.forEach((extension) => {
      disposables.push(
        registerCompletionProvider(extension, /class=["|']([\w- ]*$)/)
      );
    });
...
const registerCSSProviders = (disposables: Disposable[]) =>
  workspace
    ...
    ?.forEach((extension) => {
      ...
      disposables.push(
        registerCompletionProvider(extension, /@apply ([.\w- ]*$)/, ".")
      );
    });
...
const registerJavaScriptProviders = (disposables: Disposable[]) =>
  workspace
    ...
    ?.forEach((extension) => {
      disposables.push(
        registerCompletionProvider(extension, /className=["|']([\w- ]*$)/)
      );
      disposables.push(
        registerCompletionProvider(extension, /class=["|']([\w- ]*$)/)
      );
...
function registerEmmetProviders(disposables: Disposable[]) {
  ...
  const registerProviders = (modes: string[]) => {
    modes.forEach((language) => {
      disposables.push(
        registerCompletionProvider(language, emmetRegex, "", ".")
      );
    });
  };
...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  workspace.onDidChangeConfiguration(
    async (e) => {
      try {
        if (
          e.affectsConfiguration(Configuration.IncludeGlobPattern) ||
  ...
  if (
    workspace.getConfiguration().get<boolean>(Configuration.EnableEmmetSupport)
  ) {
    registerEmmetProviders(emmetDisposables);
  }
